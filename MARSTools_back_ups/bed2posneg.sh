...
function get_fasta(){
    ...
    #check the size of the input sequence o reduce the processing time form large files (assumes sorted file)
    ...
    if [ "$lenbed" -gt 5000 ];
        then
            shuffleBed -i $bed_in -g /home/kipkurui/Downloads/human.hg19.genome | cut -f 1,2,3 | head -$lenuse | bed-widen -width $len  >$bed_wide.bed
     else
            shuffleBed -i $bed_in -g /home/kipkurui/Downloads/human.hg19.genome | cut -f 1,2,3 | bed-widen -width 100  >$bed_wide.bed
    fi
    ...
    head -$lenuse $bed_in | cut -f 5 >/tmp/f1
